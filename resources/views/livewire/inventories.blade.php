<div>
    <div class="row">
        <div class="col-8">
            ...
            <form target="_blank" class="mt-3" action="{{ route('inventory.exportExcel') }}" method="POST"
                enctype="multipart/form-data" id="pot">
                @csrf
        ...
        <tbody>
            @foreach ($data as $value)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('inventory.edit', ['id' => $value->id]) }}" class="btn btn-primary btn-sm"> <i
                                class="fa fa-edit"></i></a>
                        @if ($value->enabled == 1)
                            <a href="{{ route('inventory.updateStatus', ['id' => $value->id, 'status' => 0]) }}"
                                class="btn btn-danger btn-sm"> <i class="fa fa-trash"></i></a>
                        @else
                            <a href="{{ route('inventory.updateStatus', ['id' => $value->id, 'status' => 1]) }}"
